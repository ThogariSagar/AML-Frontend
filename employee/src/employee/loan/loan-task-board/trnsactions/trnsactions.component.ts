...
@Component({
  ...
  styleUrls: ['./trnsactions.component.css']
})
export class TrnsactionsComponent implements OnInit {
// data: any;
data: any[] = [];
...
submit() {
  ...
  this.http.post<any[]>('http://localhost:8080/loans/readTransactionsCsv', formData).subscribe(
    response => {
       this.data = response;
       this.cdr.detectChanges();
...
saveData() {
  ...
  this.http.post<any[]>(`http://localhost:8080/loans/saveTxnsData/${this.id}`, this.data).subscribe(    
  response => {
